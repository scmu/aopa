...
split-meet-⊑ : ∀ {i j} {A : Set i} {B : Set j} {C : Set j}
             → (R : B ← A ⊣ j) → (S : C ← A ⊣ j)
             → ⟨ R , S ⟩ ⊑ ((fun proj₁)˘ ○ R) ⊓ ((fun proj₂) ˘ ○ S)
split-meet-⊑ R S (b , c) a (bRa , cSa) = (b , bRa , refl) , (c , cSa , refl)

split-meet-⊒ : ∀ {i j} {A : Set i} {B : Set j} {C : Set j}
             → (R : B ← A ⊣ j) → (S : C ← A ⊣ j)
             → ⟨ R , S ⟩ ⊒ ((fun proj₁)˘ ○ R) ⊓ ((fun proj₂) ˘ ○ S)
split-meet-⊒ R S (b , c) a ((._ , bRa , refl) , (._ , cSa , refl)) = bRa , cSa

split-meet : ∀ {i j} {A : Set i} {B : Set j} {C : Set j}
           → (R : B ← A ⊣ j) → (S : C ← A ⊣ j)
           → ⟨ R , S ⟩ ≑ ((fun proj₁)˘ ○ R) ⊓ ((fun proj₂) ˘ ○ S)
split-meet R S = (split-meet-⊑ R S , split-meet-⊒ R S)
