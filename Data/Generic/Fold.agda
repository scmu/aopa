...
mutual
 ...
 mapFold-univ-⇐ one h f hom tt = refl
 mapFold-univ-⇐ arg₁ h f hom (fst a) = refl
...
mutual
  ...
  mapFold-univ-⇒ zer h f hom ()
  mapFold-univ-⇒ one h f hom tt = refl
  mapFold-univ-⇒ arg₁ h f hom (fst x) = refl
  mapFold-univ-⇒ arg₂ h f hom (snd y) = cong snd (sym (hom y))
  mapFold-univ-⇒ (G₁ ⊕ G₂) h f hom (inj₁ x) = cong inj₁ (mapFold-univ-⇒ G₁ h f hom x)
  mapFold-univ-⇒ (G₁ ⊕ G₂) h f hom (inj₂ y) = cong inj₂ (mapFold-univ-⇒ G₂ h f hom y)
  mapFold-univ-⇒ (G₁ ⊗ G₂) h f hom (x , y)
    rewrite mapFold-univ-⇒ G₁ h f hom x | mapFold-univ-⇒ G₂ h f hom y = refl
...
-- computation rules:
--  foldR F R ○ fun In ≑ R ○ fmapR F (foldR F R)

foldR-computation-mf-⊑ : {F : PolyF} → {A B : Set}
                     → (R : B ← ⟦ F ⟧ A B)
                     → ⦇ R ⦈ ○ fun In ⊑ R ○ mapFoldR F F R
foldR-computation-mf-⊑ R b xs (._ , refl , p) = p

foldR-computation-mf-⊒ : {F : PolyF} → {A B : Set}
                     → (R : B ← ⟦ F ⟧ A B)
                     → ⦇ R ⦈ ○ fun In ⊒ R ○ mapFoldR F F R
foldR-computation-mf-⊒ R b xs p = In xs , refl , p
...
foldR-computation : {F : PolyF} → {A B : Set}
                  → (R : B ← ⟦ F ⟧ A B)
                  → (⦇ R ⦈ ○ fun In ≑ R ○ fmapR F ⦇ R ⦈)
foldR-computation R = foldR-computation-⊑ R , foldR-computation-⊒ R


foldR-computation'-⊑ : {F : PolyF} → {A B : Set}
                     → (R : B ← ⟦ F ⟧ A B)
                     → ⦇ R ⦈ ⊑ R ○ fmapR F ⦇ R ⦈ ○ fun In ˘
foldR-computation'-⊑ {F} R =
  ⊑-begin
    ⦇ R ⦈
  ⊑⟨ refl-elim-r In-surjective ⟩
    ⦇ R ⦈ ○ fun In ○ fun In ˘
  ⊑⟨ ⇦-mono-l (⦇ R ⦈ ● fun In ‥) (R ● fmapR F ⦇ R ⦈ ‥) (foldR-computation-⊑ R) ⟩
    R ○ fmapR F ⦇ R ⦈ ○ fun In ˘
  ⊑∎
